...
function cast_oTailentPool(tePersonelReserve)
{
    ...
    var oItem = {
        ...
        link: 'personnel_reserve/' + tePersonelReserve.id.Value
    };
...
function GetAdaptation(iPersonIDParam, sPersonType, Statuses, sAccessType, sApplication, filter, oCollectionParams, oCalcProblemParams)
{
    ...
    try
    {
        ...
        arrConds.push("$elem/position_type = 'adaptation' or $elem/position_type = 'position_common'");
...
//toLog("GetAdaptation: strReq: " + strReq);
        ...
        var sReqBossType, fldStatus, fldSubdivision, link;
...
/**
 * @function GetAdaptationHierStageTaskByID
 * @memberof Websoft.WT.TalentPool
 * @author BG
 * @description Получение иерархического списка этапов и задач программы развития в этапе развития карьеры.
 * @param {bigint} iCareerReserveIDParam - ID этапа развития карьеры
 ...
 * @returns {WTReserveTaskResult}
 */
function GetAdaptationHierStageTaskByID(iCareerReserveIDParam, oCollectionParams, oAccessParam)
{
    var oRet = tools.get_code_library_result_object();
    oRet.result = [];
    oRet.data = {};
...
function GetBossesByPerson(iObjectIDParam, oAccessParam)
{
    ...
    //Непосредственные руководители без флага is_native #54910
    var arrMainBosses = XQuery("for $elem in func_managers where object_id = " + iPersonID + " and boss_type_id = 6148914691236517290 and is_native = 'false' return $elem");
    var bossQuery
    ...
    for (itemMainBoss in arrMainBosses) {
        if (ArrayOptFind(arrBosses, 'id == ' + itemMainBoss.person_id.Value ) == undefined) {
            bossQuery = ArrayOptFirstElem(XQuery("for $elem in collaborators where id = " + itemMainBoss.person_id.Value + " return $elem"));
            oRet.result.push(cast_oCollaborator(bossQuery));
        }
    }
...
function GetSubordinateTalentPoolContext(PersonIDParam)
{
    ...
    try
    {
        ...
        var libMainParam = tools.get_params_code_library('libMainPetrovich');
        ...
        var arrPersons = tools.call_code_library_method( "libMainPetrovich", "GetTypicalSubordinates", [ iPersonID, libMainParam.GetOptProperty("DefaultSubordinateType"), libMainParam.GetOptProperty("iBossTypeIDs"), false, (!tools_web.is_true(libMainParam.GetOptProperty("bRetunDismissedPerson", false)) ? "$elem/is_dismiss != true()" : "") ] );
...
function OpenCareerReserveTask( iCurUserID, iCareerReserveActID, sTaskID, bNewWindow, bCanChangeTask, bCanChangeBeforeDate, sLink, sCommand, SCOPE_WVARS, curLngWeb )
{
    ...
    var tutor = 5703809445382982252;
    var boss = 6148914691236517290;
    ...
    isTutor = ArrayOptFind(teCareerReserve.tutors, "This.person_id == "+iCurUserID+" && (This.boss_type_id == "+tutor+" || This.boss_type_id == "+boss+")") != undefined
    ...
    switch( sCommand )
    {
        case "eval":
            ...
            if ( ! bCanChangeTask )
            {
                ...
                else
                {
                    ...
                    {
                        if (isTutor || teCareerReserve.person_id == iCurUserID) {
                            oRes.action_result = {
                                command: "new_window",
                                url: sLink
                            };
            ...
            else
            {
                ...
                {
                    var reserves = ArrayOptFirstElem(XQuery("sql:\
						SELECT\
							cr.data.value('(career_reserve/tasks/task[id=\"" + sTaskID + "\"]/person_comment)[1]', 'NVARCHAR(MAX)') AS person_comment,\
							cr.data.value('(career_reserve/tasks/task[id=\"" + sTaskID + "\"]/tutor_comment)[1]', 'NVARCHAR(MAX)') AS tutor_comment\
						FROM\
							career_reserve cr\
						JOIN\
							career_reserves crs ON cr.id = crs.id\
						WHERE\
							cr.id = " + iCareerReserveActID + "\
							AND (\
								cr.data.value('(career_reserve/tasks/task[id=\"" + sTaskID + "\"]/person_comment)[1]', 'NVARCHAR(MAX)') IS NOT NULL\
								OR\
								cr.data.value('(career_reserve/tasks/task[id=\"" + sTaskID + "\"]/tutor_comment)[1]', 'NVARCHAR(MAX)') IS NOT NULL\
							);\
					"));
                    ...
                    if ((fldTask.object_type == 'task_type' || fldTask.object_type == 'library_material' || fldTask.type_document == 'library_material') && fldTask.status == 'passed') {
                    
                        if (fldTask.object_type == 'task_type') {
                            oRes.action_result.form_fields.push(
                                {
                                    name: "p1",
                                    type: "paragraph",
                                    value: "<h4 style='padding: 0;margin: 0;'>" + fldTask.name + " </h4>"
                                },
                                {
                                    name: "p1",
                                    type: "paragraph",
                                    value: "<p style='padding: 0;margin: 0;'>" + fldTask.desc + " </p>"
                                }
                            );
                        }else{
                            oRes.action_result.form_fields.push(
                                {
                                    name: "p1",
                                    type: "paragraph",
                                    value: '<h4> <a href="/_wt/' + fldTask.object_id + '" style="padding: 0; margin: 0;">' + fldTask.name + '</a> </h4>'
                                }
                            );
                        }
                    }
                    ...
                    if (reserves != undefined) {
                        if (reserves.person_comment != undefined && reserves.person_comment != '') {
                            oRes.action_result.form_fields.push({
                                name: "p1",
                                type: "paragraph",
                                value: "<p style='padding: 0;margin: 0;'> Комментарий сотрудника: " + reserves.person_comment + " </p>"
                            });
                        }

                        if (reserves.tutor_comment != undefined && reserves.tutor_comment != '') {
                            oRes.action_result.form_fields.push({
                                name: "p2",
                                type: "paragraph",
                                value: "<p style='padding: 0;margin: 0;'> Комментарий наставника: " + reserves.tutor_comment + " </p>"
                            });
                        }
                    }
                ...
                else if ( fldTask.status == 'plan' || fldTask.status == 'active' )
                {
                    ...
                    if ( ! bCanChangeBeforeDate && fldTask.start_date.HasValue && DateDiff( Date(), Date( fldTask.start_date.Value ) ) < 0 )
                    {
                        ...
                        if ((fldTask.object_type == 'task_type' || fldTask.object_type == 'library_material' || fldTask.type_document == 'library_material')) {
                            ...
                            oRes.action_result = {
                                ...
                                buttons:
                                    [
                                        { name: "cancel", label: "Закрыть", type: "cancel" }
                                    ]
                            };
                            ...
                            oRes.action_result.form_fields.push(
                                {
                                    name: "p1",
                                    type: "paragraph",
                                    value: '<center> <h4>Внимание! Дата начала работы с задачей еще не наступила</h4> </center>'
                                }
                            );
        ...
        case "submit_form":
            ...
            if ( sTaskID != '' )
            {
                ...
                if ( fldTask != undefined )
                {
                    var user = ArrayOptFindByKey(teCareerReserve.tutors, iCurUserID, 'person_id')
                    if (user != undefined) {
                        if (user.boss_type_id.Value == 6148914691236517290 || user.boss_type_id.Value == 5703809445382982252) {
                            fldTask.tutor_comment = sComment;
                        } else {
