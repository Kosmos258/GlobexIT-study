...
function cast_oReserveTask(itemTask, iCurUserID, ObjectParam)
{
    ...
    var iTutorID = null;
    var sTutorFullname = "";
    var sImgUrl = "/ico/new_icons/" + itemTask.type.Value + ".ico";
    var sDeadlines = "-"
    ...
    if(itemTask.object_id.HasValue)
    {
        ...
        if(exch_type != undefined)
        {
            ...
            if(objName != undefined )
            {
                ...
                {
                    //#50163 05.11.2024 - убрана смена иконки для ресурсов, добавляется по типу, как и для всех остальных
    ...
    var oItem = {
        ...
        start_date_str_short: (itemTask.ChildExists("start_date") ? StrDate(itemTask.start_date.Value, false) : ""),
        plan_date: itemTask.plan_date.Value,
        plan_date_str: StrLongDate(itemTask.plan_date.Value),
        plan_date_str_short: StrDate(itemTask.plan_date.Value, false),
        fact_date: itemTask.fact_date.Value,
        fact_date_str: StrLongDate(itemTask.fact_date.Value),
        fact_date_str_short: StrDate(itemTask.fact_date.Value, false),
        deadline: sDeadlines,
        ...
        is_my_task: 'нет',
        task_test: '', // Задача куратора и руководителя, Задача руководителя, Задача куратора
        last_try: ''
    };
...
function GetCareerReserveTasksByPerson(sCareerReserveType, iPersonID, oCollectionParams, isMyTasksVar)
{
    ...
    oRet.data = {};
    oRet.paging = oCollectionParams.paging;
    sCareerReserveType = 'position_common'
    try
    {
    ...
    if(isBoss) {
        for (task in allTasks) {
            if(task.is_lead_task == true && task.is_mentor_task == true) {
                task.task_test = 'Задача куратора и руководителя';
            }
            ...
            if(task.is_lead_task == false && task.is_mentor_task == true) {
                task.task_test = 'Задача куратора';
            }

            if (OptInt(boss_type_id) == OptInt(boss_id)) {
                if(task.is_lead_task == true) {
                    task.is_my_task = 'да'
                }
                if (isMyTasksVar && task.is_lead_task == true) {
                    oRet.result.push(task)
                }
                if(!isMyTasksVar && (task.is_lead_task == true || task.is_mentor_task == true || (task.is_mentor_task == false && task.is_lead_task == false))) {
                    oRet.result.push(task)
                }
            } else if (OptInt(boss_type_id) == OptInt(admin_id)) {
                oRet.result.push(task)
            } else {
                if(task.is_mentor_task == true) {
                    task.is_my_task = 'да'
                }
                if (isMyTasksVar && task.is_mentor_task == true) {
                    oRet.result.push(task)
                }
                if(!isMyTasksVar && (task.is_mentor_task == true || (task.is_mentor_task == false && task.is_lead_task == false))) {
                    oRet.result.push(task)
                }
            }
...
/**
 * @function GetAdaptationTasksByPerson
 * @memberof Websoft.WT.TalentPool
 * @author BG
 * @description Получение списка задач программы развития в этапе развития карьеры по ID сотрудника.
 * @param {string} sCareerReserveType - тип кадрового резерва
 ...
 * @returns {WTReserveTaskResult}
 */
function GetAdaptationTasksByPerson(sCareerReserveType, iPersonID, oCollectionParams, isMyTasksVar)
{
    //alert('GetAdaptationTasksByPerson')
    var boss_id = '6148914691236517290'
    var admin_id = '7000156729325367608'
    var superviser_tutor = '7105332651422086498'
    var oRet = tools.get_code_library_result_object();
    oRet.result = [];
    oRet.data = {};
    oRet.paging = oCollectionParams.paging;
    ...
    if(isBoss) {
        for (task in allTasks) {
            ...
            } else if (OptInt(boss_type_id) == OptInt(admin_id) || OptInt(boss_type_id) == OptInt(superviser_tutor)) {
                oRet.result.push(task)
            } else {
                if(task.is_mentor_task == true) {
                    task.is_my_task = 'да'
...
function GetCareerReserveHierStageTaskByID(iCareerReserveIDParam, oCollectionParams, oAccessParam)
{
    ...
    var bIsCareerReserve = false;

    if (docCareerReserve != undefined && docCareerReserve.TopElem.position_type == "position_common") {
        bIsCareerReserve = true;
    }
    ...
    for (task in ALL_TASKS) {
        if (isBoss) {
            ...
            if (OptInt(boss_type_id) == OptInt(boss_id)) {
                if (task.is_lead_task == true || task.is_mentor_task == true || (task.is_mentor_task == false && task.is_lead_task == false)) {
                    oRet.result.push(task)
                }
            } else if (OptInt(boss_type_id) == OptInt(admin_id)) {
                oRet.result.push(task)
...
function AddTaskCareerReserve(iCareerReserveIDParam, sCommand, sFormFields, iCurUserID, oScopeWvars )
{
    ...
    try
    {
        ...
        if (teCareerReserve.position_type == "position_common") {
            bIsCareerReserve = true;
        }
        ...
        if(sCommand == "eval" || (sCommand == "submit_form" && sNextStep == "form"))
        {
            ...
            switch( sSendNotificationType )
            {
                ...
                default:
                    ...
                    var isTutor = tools.is_user_boss(OptInt(iCurUserID), OptInt(teCareerReserve.person_id), 'not_native', tutorBossTypeID)
                    var isBoss = tools.is_boss(OptInt(iCurUserID), OptInt(teCareerReserve.person_id))
                    if (isTutor) {
                        recipientsArr.push({ name: "Сотрудник", value: "collaborator" })
                    } else if (isBoss) {
                        recipientsArr.push({ name: "Сотрудник", value: "collaborator" })

                        if (bIsCareerReserve) {
                            recipientsArr.push({ name: "Кураторы", value: "tutor" })
                        } else {
                            recipientsArr.push({ name: "Наставники", value: "tutor" })
                        }
                    }
                    var oSendTypeField = {
                        name: "send_type",
